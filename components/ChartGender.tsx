 ...
 const ChartGender:React.FC<StateProps> = ({state,stateData,name}) => {
    ...
    const options = {
        ...
        data: [
        {
            ...
            type: "column",
            dataPoints: [
                { label: `Men ${((gender.men.length * 100)/filteredData.data.length).toFixed(1)}%`, y: gender.men.length },
                { label: `Women ${((gender.women.length * 100)/filteredData.data.length).toFixed(1)}%`, y: gender.women.length  },

...
const Chart: React.FC<StateProps> = ({ state, stateData, name }) => {
  ...
  useEffect(() => {
    ...
    const hispanic = filteredData.filter(
      (a: any) => a.race === "H" || a.race === "N"
    );
    ...
    data: [
      {
        ...
        dataPoints: [
          {
            label: `Black ${(
              (black.black.length * 100) /
              filteredData.data.length
            ).toFixed(1)}%`,
            y: black.black.length,
          },
          {
            label: `White ${(
              (white.white.length * 100) /
              filteredData.data.length
            ).toFixed(1)}%`,
            y: white.white.length,
          },
          {
            label: `Asian ${(
              (asian.asian.length * 100) /
              filteredData.data.length
            ).toFixed(1)}%`,
            y: asian.asian.length,
          },
          {
            label: `Hispanic ${(
              (hispanic.hispanic.length * 100) /
              filteredData.data.length
            ).toFixed(1)} %`,
            y: hispanic.hispanic.length,
          },
          {
            label: `Other ${(
              (other.other.length * 100) /
              filteredData.data.length
            ).toFixed(1)}%`,
            y: other.other.length,
          },
          {
            label: `Unknown ${(
              (unknown.unknown.length * 100) /
              filteredData.data.length
            ).toFixed(1)}%`,
            y: unknown.unknown.length,
          },
        ],
      },
    ],
